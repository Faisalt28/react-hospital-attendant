    ...
    <section className="py-20 lg:py-28">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-14 grid gap-5 text-center md:grid-cols-2 md:text-left items-end">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-gray-900 dark:text-gray-100 tracking-tight leading-tight">
            {title}
          </h1>
        ...
        <div className="grid gap-7 lg:grid-cols-3 items-stretch">
          <div className="relative group overflow-hidden rounded-3xl lg:col-span-2 shadow-lg min-h-[360px] lg:min-h-[500px]">
            <img
              src={mainImage.src}
              alt={mainImage.alt}
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent flex items-end p-6 sm:p-8">
              <div className="text-white space-y-1">
                <span className="px-3 py-1 bg-emerald-600/90 text-white rounded-full text-xs font-semibold backdrop-blur-xs">
                  Fasilitas & Sistem Terintegrasi
                </span>
                <p className="text-lg font-bold">Layanan Kesehatan 24 Jam Nonstop</p>
                <p className="text-xs text-gray-200">Didukung pemantauan jadwal sif dan kesiapan dokter jaga secara presisi.</p>
              </div>
            </div>
          </div>

          <div className="flex flex-col gap-7 md:flex-row lg:flex-col justify-between">
            <div className="flex flex-col justify-between gap-5 rounded-3xl bg-emerald-50/80 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-800/60 p-7 md:w-1/2 lg:w-auto shadow-sm">
              ...
              <div>
                <p className="mb-2 text-lg font-bold text-gray-900 dark:text-gray-100">{breakout.title}</p>
                <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{breakout.description}</p>
              </div>
              <Button variant="default" className="mr-auto cursor-pointer" asChild>
            ...
            <div className="relative overflow-hidden rounded-3xl grow basis-0 md:w-1/2 lg:min-h-0 lg:w-auto shadow-sm group">
              <img
                src={secondaryImage.src}
                alt={secondaryImage.alt}
                className="size-full min-h-[220px] max-h-[280px] rounded-3xl object-cover transition-transform duration-500 group-hover:scale-105"
              />
        ...
        <div className="py-20">
          <p className="text-center text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">
            {companiesTitle}
          </p>
          <div className="mt-8 flex flex-wrap justify-center items-center gap-6 sm:gap-10">
            {companies.map((company, idx) => (
              <div
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-xs text-xs font-bold text-gray-700 dark:text-gray-300"
                key={company.alt + idx}
              >
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-gray-900 via-indigo-950 to-emerald-950 text-white p-8 sm:p-14 shadow-2xl">
          <div className="flex flex-col gap-3 text-center md:text-left relative z-10">
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold tracking-tight">{achievementsTitle}</h2>
            <p className="max-w-screen-sm text-sm sm:text-base text-gray-300">
              {achievementsDescription}
            </p>
          </div>

          <div className="mt-10 grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 text-center relative z-10">
            {achievements.map((item, idx) => (
              <div className="flex flex-col gap-2 p-4 rounded-2xl bg-white/5 backdrop-blur-xs border border-white/10" key={item.label + idx}>
